...
#define HEADER_SCRIPT	"#!/bin/sh\n" \
						...
						"filename=\"$1\"\n" \
						"vim_path=\"$(whereis vim | awk -F ':' '{print $2}')\"\n" \
						"\n" \
						"install_script() {\n" \
						...
						"then\n" \
						"\tcat <<-VIM\n" \
						"\t\tvim command is not installed, you need vim to use the dynamic header\n" \
						...
						"\t\texit 4\n" \
						"\tfi\n" \
						"\tcurl -fsSL https://raw.githubusercontent.com/42Paris/42header/master/plugin/stdheader.vim -o \"$plugin_path\"\n" \
						"\tif [ ! -f $plugin_path ]\n" \
						"\tthen\n" \
						...
						"\t\t\tto your $HOME/.vimrc file if not already done or to create one \\\n" \
						...
						"fi\n" \
						"\n" \
						"vim \"+set binary\" \"+set noeol\" \"+Stdheader\" \"+wq\" $filename\n" \
						"exit 0\n"
#endif
